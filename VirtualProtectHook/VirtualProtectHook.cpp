...
decltype(&VirtualProtect) original_VirtualProtect;

static BOOL WINAPI hook_VirtualProtect(__in LPCWSTR lpPathName)
{
	dlogp("'%S'", lpPathName);
	return original_VirtualProtect(lpPathName);
}
